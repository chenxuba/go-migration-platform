...
func (svc *Service) BuildLessonHourOrderImportTemplate(userID int64) (string, error) {
	...
	columns := buildLessonHourOrderImportColumns(channels, courseNames, staffNames, orderTagNames)
	filename := sanitizeTemplateFileName(fmt.Sprintf("%s导入学员订单模板-按课时-%s.xlsx", orgName, time.Now().Format("20060102")))
	content, err := buildLessonHourOrderImportTemplateWorkbook(orgName, columns)
	if err != nil {
		return "", err
	}
...
func buildLessonHourOrderImportColumns(channels []model.ChannelVO, courseNames, staffNames, orderTagNames []string) []model.IntentionStudentImportTemplateColumn {
	...
	return []model.IntentionStudentImportTemplateColumn{
		...
		{Title: "渠道", FieldType: 4, Options: channelOptions},
		{Title: "性别", FieldType: 4, Options: []string{"男", "女", "未知"}},
		{Title: "生日", FieldType: 3},
		{Title: "微信号", FieldType: 1},
		{Title: "报读课程", Required: true, FieldType: 4, Options: courseNames},
		{Title: "报读班级", FieldType: 1},
		{Title: "购买课时数", Required: true, FieldType: 2},
		{Title: "赠送课时数", FieldType: 2},
		{Title: "已上课时数", FieldType: 2},
		{Title: "有效期至", FieldType: 3},
		{Title: "实收金额", Required: true, FieldType: 2},
		{Title: "欠费金额", FieldType: 2},
		{Title: "经办日期", FieldType: 3},
		{Title: "收款方式", FieldType: 4, Options: []string{"微信", "支付宝", "银行转账", "POS机", "现金", "其他"}},
		{Title: "收款账户", FieldType: 4, Options: []string{"默认账户"}},
		{Title: "支付单号", FieldType: 1},
		{Title: "对方账户", FieldType: 1},
		{Title: "订单备注", FieldType: 1},
		{Title: "订单标签", FieldType: 4, Options: orderTagNames},
		{Title: "是否为体验价", FieldType: 4, Options: []string{"是", "否"}},
		{Title: "销售", FieldType: 4, Options: staffNames},
		{Title: "订单销售员", FieldType: 4, Options: staffNames},
	}
}

func buildLessonHourOrderImportTemplateWorkbook(orgName string, columns []model.IntentionStudentImportTemplateColumn) ([]byte, error) {
	...
	for idx, column := range columns {
		...
		file.SetColWidth(sheetName, col, col, lessonHourOrderTemplateColumnWidth(column.Title))
	...
	if err := file.SetCellRichText(sheetName, noteStartCell, buildLessonHourOrderImportNotesRichText(orgName, columns)); err != nil {
		return nil, err
	}
...
func lessonHourOrderTemplateColumnWidth(title string) float64 {
	switch strings.TrimSpace(title) {
	case "学员姓名", "手机号", "手机号归属人", "渠道", "性别", "生日", "微信号":
		return 16
	case "报读课程", "报读班级":
		return 22
	case "购买课时数", "赠送课时数", "已上课耗数", "有效期至":
		return 16
	case "实收金额", "欠费金额", "经办日期":
		return 14
...
func buildLessonHourOrderImportNotesRichText(orgName string, columns []model.IntentionStudentImportTemplateColumn) []excelize.RichTextRun {
	...
	runs := []excelize.RichTextRun{
		{Text: "导入学员订单-按课时模板，用于向", Font: black},
	...
	runs = append(runs,
		excelize.RichTextRun{Text: "3、「手机号」必须为 1 开头的 11 位数字，不支持“-”和中间空格，例如 13311113333。\n", Font: black},
		excelize.RichTextRun{Text: "4、「生日」、「经办日期」、「有效期至」支持 2021-01-21、2021/01/21 等可识别日期格式。\n", Font: black},
		excelize.RichTextRun{Text: "5、「报读课程」请从下拉框选择当前机构内已创建的课程名称。\n", Font: black},
		excelize.RichTextRun{Text: "6、「收款方式」、「收款账户」、「支付单号」、「对方账户」仅在存在实收金额时填写。\n", Font: black},
		excelize.RichTextRun{Text: "7、「订单标签」、「销售」、「订单销售员」如需填写，请使用系统中已存在的名称。\n", Font: black},
		excelize.RichTextRun{Text: "8、「是否为体验价」请填写“是”或“否”。\n", Font: black},
		excelize.RichTextRun{Text: "9、最多导入1000条数据，请控制导入数量。\n", Font: black},
	)
