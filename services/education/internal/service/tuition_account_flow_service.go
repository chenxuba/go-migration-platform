...
func (svc *Service) ExportSubTuitionAccountFlowRecordList(userID int64, query model.SubTuitionAccountFlowRecordListQueryDTO) ([]byte, string, error) {
	instID, err := svc.repo.FindInstIDByUserID(context.Background(), userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, "", errors.New("no institution context")
	...
	result, err := svc.repo.GetSubTuitionAccountFlowRecordList(context.Background(), instID, exportQuery)
	if err != nil {
		return nil, "", err
	}
	if result.Total == 0 || len(result.List) == 0 {
		return nil, "", errors.New("没有符合条件的学费变动明细可以导出")
	}
	if result.Total > tuitionAccountFlowExportMaxRows {
		return nil, "", errors.New("当前列表最多支持导出10000条数据，请缩小筛选范围后重试")
	}

	content, err := buildSubTuitionAccountFlowExportWorkbook(result.List)
	if err != nil {
		return nil, "", err
	}
	fileName := fmt.Sprintf("学费变动明细-%s.xlsx", time.Now().Format("20060102150405"))
	return content, fileName, nil
}
