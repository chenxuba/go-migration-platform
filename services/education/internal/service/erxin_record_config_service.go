...
type ERXinAssessmentRecordConfigInput struct {
	ExaminerName   string
	AssessmentDate time.Time
}

func (svc *Service) UpdateERXinAssessmentRecordConfig(userID, recordID int64, input ERXinAssessmentRecordConfigInput) (model.AssessmentRecordDetailVO, error) {
	if svc.repo == nil {
	...
	if err := svc.syncERXinSubmittedDraftRecordConfig(instID, operatorID, recordID, examinerName, assessmentDate); err != nil {
		return model.AssessmentRecordDetailVO{}, err
	}
...
func (svc *Service) syncERXinSubmittedDraftRecordConfig(instID, operatorID, recordID int64, examinerName string, assessmentDate time.Time) error {
...
func dateOnlyERXinRecordConfig(value time.Time) time.Time {
	if value.IsZero() {
		return value
	}
