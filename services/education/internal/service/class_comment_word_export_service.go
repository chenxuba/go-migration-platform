...
func buildClassCommentWordDocument(items []rehabRecordWordExportView) string {
	...
	for index, item := range items {
		...
		builder.WriteString(`<td class="label date-label date-center" align="center" width="56" style="mso-width-source:userset;mso-width-alt:1120;width:56pt;white-space:nowrap;">出生年月</td><td class="date-left nowrap" align="left" width="78" style="mso-width-source:userset;mso-width-alt:1560;width:78pt;white-space:nowrap;">` + renderWordText(item.BirthDate) + `</td>`)
		builder.WriteString(`<td class="label-multi" width="24" style="mso-width-source:userset;mso-width-alt:480;width:24pt;">班<br>别</td><td class="left class-name-cell" width="95" style="mso-width-source:userset;mso-width-alt:1900;width:105pt;">` + renderWordText(item.ClassName) + `</td>`)
		builder.WriteString(`</tr>`)
		builder.WriteString(`<tr>`)
		builder.WriteString(`<td class="label-multi">任教<br>老师</td><td colspan="3" class="left">` + renderWordText(item.TeacherName) + `</td>`)
		builder.WriteString(`<td class="label date-label date-center" align="center">训练日期</td><td colspan="3" class="date-left nowrap" align="left">` + renderWordText(item.TrainingDate) + `</td>`)
		builder.WriteString(`</tr>`)
		builder.WriteString(`<tr>`)
		...
		builder.WriteString(`<tr style="height:48pt;mso-height-rule:at-least;"><td class="label-multi">康复<br>建议</td><td colspan="7" class="left top advice-block" style="height:48pt;">` + renderWordText(item.Suggestion) + `</td></tr>`)
...
func renderSignatureDateRow(item rehabRecordWordExportView) string {
	var builder strings.Builder
	builder.WriteString(`<p class="signature-line" style="margin:0;white-space:nowrap;tab-stops:right 350pt;">`)
	builder.WriteString(`<span class="signature-prefix">家长签名：</span>` + renderSignatureHTML(item))
	builder.WriteString(`<span style="mso-tab-count:1"></span>`)
	builder.WriteString(`<span class="date-inline">` + renderFeedbackDateHTML(item.FeedbackDate) + `</span>`)
