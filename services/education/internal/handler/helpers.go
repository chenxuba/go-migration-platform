...
func parseRegistrationListQueryDTO(raw map[string]any) model.RegistrationListQueryDTO {
	...
	if qm, ok := raw["queryModel"].(map[string]any); ok {
		query.QueryModel = model.RegistrationListFilters{
			...
			HasAssignedClassCourse:     asBoolPtr(qm["hasAssignedClassCourse"]),
			StudentID:                  asString(qm["studentId"]),
...
func parseUpdateStudentTeachingRecordDTO(raw map[string]any) model.UpdateStudentTeachingRecordDTO {
	return model.UpdateStudentTeachingRecordDTO{
		...
		Remark:                  asString(raw["remark"]),
		ExternalRemark:          asString(raw["externalRemark"]),
	}
}
...
func formatIntentStudentDetail(item model.IntentStudent) map[string]any {
	result := map[string]any{
		...
		"supervisorAssignedTime":      formatNullableDateTime(item.SupervisorAssignedTime),
