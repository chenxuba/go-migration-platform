...
func (handler *Handler) parentWeChatOfficialStatus(w http.ResponseWriter, r *http.Request) {
	ctx := tenant.FromContext(r.Context())
	claims, ok := handler.requireParentAuth(w, r, ctx)
	if !ok {
		return
	...
	result, err := handler.service.GetParentWeChatOfficialStatusByPhone(r.Context(), claims.Username)
	if err != nil {
		httpx.WriteError(w, http.StatusBadRequest, err.Error(), ctx.RequestID)
		return
	}
