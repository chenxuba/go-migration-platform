...
type rechargeAccountImportTaskEntity struct {
...
func (repo *Repository) CreateRechargeAccountImportTask(ctx context.Context, instID int64, detail model.IntentionStudentImportTaskDetail, columns []model.IntentionStudentImportColumn, rows []model.IntentionStudentImportRow) error {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO recharge_account_import_task (
	...
	for _, row := range rows {
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO recharge_account_import_task_record (
...
func (repo *Repository) GetRechargeAccountImportTask(ctx context.Context, taskID string) (rechargeAccountImportTaskEntity, error) {
	entity := rechargeAccountImportTaskEntity{}
	var (
	...
	err := repo.db.QueryRowContext(ctx, `
		...
		FROM recharge_account_import_task
	...
	if err != nil {
		return rechargeAccountImportTaskEntity{}, err
	}
	if executeID.Valid {
	...
	rows, err := repo.db.QueryContext(ctx, `
		...
		FROM recharge_account_import_task_record
...
func (repo *Repository) ListRechargeAccountImportTasks(ctx context.Context, instID int64) (model.IntentionStudentImportTaskListResult, error) {
	rows, err := repo.db.QueryContext(ctx, `
		SELECT id, file_name, upload_staff_id, upload_staff_name, execute_staff_id, execute_staff_name,
		       total_rows, executed_rows, deleted_rows, error_rows, created_time, confirm_time, complete_time,
		       status, inst_name
...
func (repo *Repository) UpdateRechargeAccountImportTask(ctx context.Context, detail model.IntentionStudentImportTaskDetail, rows []model.IntentionStudentImportRow) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if _, err := repo.updateRechargeAccountImportTaskDetailTx(ctx, tx, detail, nil); err != nil {
		return err
	}
	for _, row := range rows {
		if err := repo.updateRechargeAccountImportTaskRowTx(ctx, tx, detail.ID, row); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (repo *Repository) MarkRechargeAccountImportTaskRunning(ctx context.Context, detail model.IntentionStudentImportTaskDetail, rows []model.IntentionStudentImportRow) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	expectedStatus := 3
	affected, err := repo.updateRechargeAccountImportTaskDetailTx(ctx, tx, detail, &expectedStatus)
	if err != nil {
		return err
	}
	if affected == 0 {
		return ErrRechargeAccountImportTaskStartConflict
	}
	for _, row := range rows {
		if err := repo.updateRechargeAccountImportTaskRowTx(ctx, tx, detail.ID, row); err != nil {
			return err
...
func (repo *Repository) UpdateRechargeAccountImportTaskProgress(ctx context.Context, detail model.IntentionStudentImportTaskDetail, row model.IntentionStudentImportRow) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (repo *Repository) updateRechargeAccountImportTaskDetailTx(ctx context.Context, tx *sql.Tx, detail model.IntentionStudentImportTaskDetail, expectedStatus *int) (int64, error) {
	query := `
		UPDATE recharge_account_import_task
...
func (repo *Repository) updateRechargeAccountImportTaskRowTx(ctx context.Context, tx *sql.Tx, taskID string, row model.IntentionStudentImportRow) error {
	cellsRaw, err := json.Marshal(row.Cells)
	if err != nil {
		return err
	}
	_, err = tx.ExecContext(ctx, `
		UPDATE recharge_account_import_task_record
...
func (repo *Repository) ClearRechargeAccountImportTasks(ctx context.Context, instID int64) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(ctx, `
		UPDATE recharge_account_import_task_record r
...
func (repo *Repository) DeleteRechargeAccountImportTask(ctx context.Context, instID int64, taskID string) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func ensureRechargeAccountImportTables(ctx context.Context, db *sql.DB) error {
	_, err := db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS recharge_account_import_task (
	...
	_, err = db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS recharge_account_import_task_record (
			id VARCHAR(64) PRIMARY KEY,
...
var errRechargeAccountImportTaskNotFound = errors.New("recharge account import task not found")
var ErrRechargeAccountImportTaskStartConflict = errors.New("recharge account import task start conflict")
