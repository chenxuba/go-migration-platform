...
func (repo *Repository) BatchUpdateOneToOneAttributes(ctx context.Context, instID, operatorID int64, ids []int64, dto model.OneToOneBatchAttributeDTO) error {
	if len(ids) == 0 {
		return nil
	}
	tx, err := repo.db.BeginTx(ctx, nil)
	...
	placeholders := make([]string, 0, len(ids))
	classArgs := make([]any, 0, len(ids)+6)
	setParts := make([]string, 0, 4)
	if dto.Status != nil {
		setParts = append(setParts, "status = ?")
		classArgs = append(classArgs, *dto.Status)
	}
	defaultTeacherID := parseInt64String(dto.DefaultTeacherID)
	if strings.TrimSpace(dto.DefaultTeacherID) != "" {
		setParts = append(setParts, "default_teacher_id = ?")
		classArgs = append(classArgs, defaultTeacherID)
	}
	if len(setParts) > 0 {
		setParts = append(setParts, "update_id = ?", "update_time = NOW()")
		classArgs = append(classArgs, operatorID)
		for _, id := range ids {
			placeholders = append(placeholders, "?")
			classArgs = append(classArgs, id)
		}
		classArgs = append(classArgs, instID, model.TeachingClassTypeOneToOne)
		if _, err := tx.ExecContext(ctx, `
			UPDATE teaching_class
	...
	if dto.ClassStudentStatus != nil {
		studentArgs := make([]any, 0, len(ids)+4)
		studentArgs = append(studentArgs, *dto.ClassStudentStatus, operatorID)
		studentPlaceholders := make([]string, 0, len(ids))
		for _, id := range ids {
			studentPlaceholders = append(studentPlaceholders, "?")
			studentArgs = append(studentArgs, id)
		}
		studentArgs = append(studentArgs, instID, model.TeachingClassTypeOneToOne)
		if _, err := tx.ExecContext(ctx, `
			UPDATE teaching_class_student tcs
			INNER JOIN teaching_class tc ON tc.id = tcs.teaching_class_id AND tc.inst_id = tcs.inst_id AND tc.del_flag = 0
			SET tcs.class_student_status = ?, tcs.update_id = ?, tcs.update_time = NOW()
			WHERE tc.id IN (`+strings.Join(studentPlaceholders, ",")+`) AND tc.inst_id = ? AND tc.class_type = ? AND tcs.del_flag = 0
		`, studentArgs...); err != nil {
			return err
		}
	}
	...
	if strings.TrimSpace(dto.DefaultTeacherID) != "" {
		...
		deletePlaceholders := make([]string, 0, len(ids))
		for _, id := range ids {
		...
		if _, err := tx.ExecContext(ctx, `
			UPDATE teaching_class_teacher
			SET del_flag = 1, update_id = ?, update_time = NOW()
			WHERE inst_id = ? AND teaching_class_id IN (`+strings.Join(deletePlaceholders, ",")+`) AND del_flag = 0
		`, append([]any{operatorID}, deleteArgs...)...); err != nil {
			return err
		}
		if defaultTeacherID > 0 {
			now := time.Now()
			for _, id := range ids {
				if _, err := tx.ExecContext(ctx, `
					...
					ON DUPLICATE KEY UPDATE
						...
						update_id = VALUES(update_id),
						update_time = VALUES(update_time)
				`, instID, id, defaultTeacherID, operatorID, now, operatorID, now); err != nil {
					return err
				}
			}
		}
