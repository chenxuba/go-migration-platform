...
type orderImportTaskEntity struct {
...
func (repo *Repository) CreateOrderImportTask(ctx context.Context, instID int64, detail model.IntentionStudentImportTaskDetail, columns []model.IntentionStudentImportColumn, rows []model.IntentionStudentImportRow) error {
	...
	for _, row := range rows {
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO order_import_task_record (
...
func (repo *Repository) GetOrderImportTask(ctx context.Context, taskID string) (orderImportTaskEntity, error) {
	entity := orderImportTaskEntity{}
	var (
	...
	if err != nil {
		return orderImportTaskEntity{}, err
	}
	if executeID.Valid {
...
func (repo *Repository) ListOrderImportTasks(ctx context.Context, instID int64) (model.IntentionStudentImportTaskListResult, error) {
	rows, err := repo.db.QueryContext(ctx, `
		SELECT id, file_name, upload_staff_id, upload_staff_name, execute_staff_id, execute_staff_name,
		       total_rows, executed_rows, deleted_rows, error_rows, created_time, confirm_time, complete_time,
		       status, inst_name
...
func (repo *Repository) UpdateOrderImportTask(ctx context.Context, detail model.IntentionStudentImportTaskDetail, rows []model.IntentionStudentImportRow) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (repo *Repository) DeleteOrderImportTask(ctx context.Context, instID int64, taskID string) error {
	tx, err := repo.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func ensureOrderImportTables(ctx context.Context, db *sql.DB) error {
	_, err := db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS order_import_task (
	...
	_, err = db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS order_import_task_record (
			id VARCHAR(64) PRIMARY KEY,
