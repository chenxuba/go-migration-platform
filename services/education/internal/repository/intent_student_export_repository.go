...
type IntentStudentExportRecordEntity struct {
	ID              int64
	InstID          int64
	ExportStaffID   int64
...
func ensureIntentStudentExportTables(ctx context.Context, db *sql.DB) error {
	_, err := db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS intent_student_export_record (
...
func (repo *Repository) CleanupExpiredIntentStudentExportRecords(ctx context.Context) error {
	_, err := repo.db.ExecContext(ctx, `
		DELETE FROM intent_student_export_record
		WHERE del_flag = 0 AND expires_at <= NOW()
	`)
	return err
}

func (repo *Repository) CreateIntentStudentExportRecord(ctx context.Context, entity IntentStudentExportRecordEntity) (int64, error) {
	queryConditionsRaw, err := json.Marshal(entity.QueryConditions)
	if err != nil {
		return 0, err
	}
	result, err := repo.db.ExecContext(ctx, `
		INSERT INTO intent_student_export_record (
...
func (repo *Repository) ListIntentStudentExportRecords(ctx context.Context, instID int64) ([]model.IntentStudentExportRecord, error) {
	rows, err := repo.db.QueryContext(ctx, `
		SELECT id, file_name, export_staff_name, total_rows, query_conditions_json, create_time, expires_at
		FROM intent_student_export_record
	...
	items := make([]model.IntentStudentExportRecord, 0, 16)
	for rows.Next() {
		var (
			item               model.IntentStudentExportRecord
...
func (repo *Repository) GetIntentStudentExportRecord(ctx context.Context, instID, recordID int64) (IntentStudentExportRecordEntity, error) {
	var (
		entity             IntentStudentExportRecordEntity
		queryConditionsRaw string
		createdTime        sql.NullTime
		expiresAt          sql.NullTime
	)
	...
	if err != nil {
		return IntentStudentExportRecordEntity{}, err
	}
	if strings.TrimSpace(queryConditionsRaw) != "" {
