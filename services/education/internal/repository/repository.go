...
func (repo *Repository) EnsureInfrastructureTables(ctx context.Context) error {
	...
	if err := ensureRefundTuitionAccountOrderTables(ctx, repo.db); err != nil {
		return err
	}
	...
	if err := ensureIntentStudentExportTables(ctx, repo.db); err != nil {
		return err
	}
	...
	if err := ensureLeaveTables(ctx, repo.db); err != nil {
		return err
	}
