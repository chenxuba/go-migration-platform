...
type teachingScheduleRollCallMeta struct {
	ScheduleID int64
	ClassType  int
	ClassID    int64
	StudentID  int64
...
type teachingScheduleRecordedStudentSummary struct {
	TeachingRecordID int64
	StudentIDSet     map[int64]struct{}
}

func normalizeTeachingScheduleCallStatus(status int) int {
	switch status {
	case 2, 3:
		return status
	default:
...
func isTeachingScheduleFullyRolled(status int) bool {
	return normalizeTeachingScheduleCallStatus(status) == 2
}

func isTeachingSchedulePartiallyRolled(status int) bool {
	return normalizeTeachingScheduleCallStatus(status) == 3
}

func (repo *Repository) loadTeachingScheduleRecordedStudentSummaryMap(ctx context.Context, queryer sqlQueryer, instID int64, scheduleIDs []int64) (map[int64]teachingScheduleRecordedStudentSummary, error) {
	if len(scheduleIDs) == 0 {
		return map[int64]teachingScheduleRecordedStudentSummary{}, nil
	}
	rows, err := queryer.QueryContext(ctx, `
		SELECT
			IFNULL(teaching_schedule_id, 0),
			IFNULL(student_id, 0),
			IFNULL(MAX(teaching_record_id), 0)
		FROM student_teaching_record
		WHERE inst_id = ?
		  AND del_flag = 0
		  AND teaching_schedule_id IN (`+sqlPlaceholders(len(scheduleIDs))+`)
		GROUP BY teaching_schedule_id, student_id
	...
	result := make(map[int64]teachingScheduleRecordedStudentSummary, len(scheduleIDs))
	for rows.Next() {
		var scheduleID, studentID, teachingRecordID int64
		if err := rows.Scan(&scheduleID, &studentID, &teachingRecordID); err != nil {
			return nil, err
		}
		if scheduleID <= 0 {
			continue
		}
		summary := result[scheduleID]
		if summary.StudentIDSet == nil {
			summary.StudentIDSet = make(map[int64]struct{})
		}
		if teachingRecordID > 0 && summary.TeachingRecordID <= 0 {
			summary.TeachingRecordID = teachingRecordID
		}
		if studentID > 0 {
			summary.StudentIDSet[studentID] = struct{}{}
		}
		result[scheduleID] = summary
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
...
func (repo *Repository) computeTeachingScheduleCallStatusMap(ctx context.Context, queryer sqlQueryer, instID int64, metas []teachingScheduleRollCallMeta) (map[int64]int, error) {
	if len(metas) == 0 {
		return map[int64]int{}, nil
	}

	scheduleIDs := make([]int64, 0, len(metas))
	groupMetas := make([]effectiveGroupClassScheduleMeta, 0, len(metas))
	for _, meta := range metas {
		if meta.ScheduleID <= 0 {
			continue
		}
		scheduleIDs = append(scheduleIDs, meta.ScheduleID)
	...
	recordedSummaryMap, err := repo.loadTeachingScheduleRecordedStudentSummaryMap(ctx, queryer, instID, uniquePositiveInt64s(scheduleIDs))
	if err != nil {
		return nil, err
	}
	...
	result := make(map[int64]int, len(metas))
	for _, meta := range metas {
		if meta.ScheduleID <= 0 {
			continue
		}
		recordedSummary := recordedSummaryMap[meta.ScheduleID]
		recordedCount := len(recordedSummary.StudentIDSet)
		status := 1

		if meta.ClassType == model.TeachingClassTypeNormal && meta.ClassID > 0 {
			expectedIDs := rosterByScheduleID[meta.ScheduleID].activeIDs()
			if len(expectedIDs) == 0 {
				if recordedCount > 0 {
					status = 2
				}
				result[meta.ScheduleID] = status
				continue
			}
			matchedCount := 0
			for _, studentID := range expectedIDs {
				if _, ok := recordedSummary.StudentIDSet[studentID]; ok {
					matchedCount++
				}
			}
			switch {
			case matchedCount <= 0:
				status = 1
			case matchedCount < len(expectedIDs):
				status = 3
			default:
				status = 2
			}
		...
		case meta.StudentID > 0:
			if _, ok := recordedSummary.StudentIDSet[meta.StudentID]; ok {
				status = 2
			}
...
func (repo *Repository) ValidateGroupClassSchedules(ctx context.Context, instID int64, dto model.CreateGroupClassSchedulesDTO) (model.TeachingScheduleValidationResult, error) {
	classID, err := strconv.ParseInt(strings.TrimSpace(dto.GroupClassID), 10, 64)
	if err != nil || classID <= 0 {
	...
	if base.Status != model.TeachingClassStatusActive {
		return model.TeachingScheduleValidationResult{}, errors.New("当前班级已结班，暂不可创建日程")
	}
	if fallbackTeacherID <= 0 {
		fallbackTeacherID = base.DefaultTeacherID
	}
	...
	for _, plan := range plans {
		for _, assistantID := range plan.AssistantIDs {
			if assistantID == plan.TeacherID {
				return model.TeachingScheduleValidationResult{}, errors.New("主教与助教不能为同一人")
			}
		}
	}
	...
	creationReferenceAt := time.Now()
	planRosterByKey := make(map[string]groupClassScheduleRoster, len(plans))
	studentConflictsByPlan := make(map[string][]scheduleConflictDetailRow, len(plans))
	conflictScheduleMetas := make([]effectiveGroupClassScheduleMeta, 0)
	seenConflictScheduleIDs := make(map[int64]struct{})
	for _, plan := range plans {
		key := schedulePlanKey(plan)
		...
		rows, err := repo.listScheduleConflictDetailsByStudentsTx(ctx, tx, instID, studentIDs, []normalizedScheduleSlot{{
			LessonDate: plan.LessonDate,
			StartAt:    plan.StartAt,
			EndAt:      plan.EndAt,
		}}, "", excludeIDs)
		...
		for _, row := range rows {
			...
			if _, ok := seenConflictScheduleIDs[row.ID]; ok {
				continue
			}
			seenConflictScheduleIDs[row.ID] = struct{}{}
			conflictScheduleMetas = append(conflictScheduleMetas, effectiveGroupClassScheduleMeta{
				ScheduleID: row.ID,
	...
	conflictRosterByScheduleID, err := repo.loadEffectiveGroupClassScheduleRosterMap(ctx, tx, instID, conflictScheduleMetas)
	if err != nil {
		return model.TeachingScheduleValidationResult{}, err
	}
	...
	classConflicts, err := repo.listScheduleConflictDetailsTx(ctx, tx, instID, "teaching_class_id", base.ClassID, plansToSlots(plans), "", excludeIDs)
	if err != nil {
		return model.TeachingScheduleValidationResult{}, err
	}

	return buildGroupClassScheduleValidationResultFromPlans(
		base,
		plans,
		planRosterByKey,
		teacherNames,
		...
		classConflicts,
		studentConflictsByPlan,
		assistantConflictsByPlan,
		conflictRosterByScheduleID,
	), nil
}
...
func (repo *Repository) GetTeachingScheduleDetail(ctx context.Context, instID int64, query model.TeachingScheduleDetailQueryDTO) (model.TeachingScheduleDetailVO, error) {
	...
	statusByID, err := repo.computeTeachingScheduleCallStatusMap(ctx, tx, instID, []teachingScheduleRollCallMeta{{
		ScheduleID: row.ID,
	...
	if len(students) == 0 && row.StudentID > 0 {
		hasTeachingRecord := strings.TrimSpace(row.TeachingRecordID) != ""
		studentCallStatus := 1
		if hasTeachingRecord {
			studentCallStatus = 2
		}
		actionDisabledReason := teachingScheduleStudentActionDisabledReason(hasTeachingRecord)
		students = []model.TeachingScheduleDetailStudentVO{{
			...
			CallStatus:              studentCallStatus,
			CallStatusText:          teachingScheduleCallStatusText(studentCallStatus),
...
func (repo *Repository) PageTeachingScheduleStudentCandidates(ctx context.Context, instID int64, dto model.TeachingScheduleStudentCandidateQueryDTO) (model.TeachingScheduleStudentCandidatePagedResult, error) {
	...
	statusByID, err := repo.computeTeachingScheduleCallStatusMap(ctx, repo.db, instID, []teachingScheduleRollCallMeta{{
		ScheduleID: row.ID,
		ClassType:  row.ClassType,
		ClassID:    row.TeachingClassID,
		StartAt:    row.StartAt,
...
func (repo *Repository) RemoveTeachingScheduleStudentCurrent(ctx context.Context, instID, operatorID int64, dto model.TeachingScheduleStudentRemoveCurrentDTO) error {
	...
	recordedSummaryMap, err := repo.loadTeachingScheduleRecordedStudentSummaryMap(ctx, tx, instID, []int64{row.ID})
	if err != nil {
		return err
	}
	if _, exists := recordedSummaryMap[row.ID].StudentIDSet[studentID]; exists {
		return errors.New("该学员已点名，不可移出本节")
	}
...
func (repo *Repository) ReplaceTeachingScheduleBatch(ctx context.Context, instID, operatorID int64, dto model.TeachingScheduleBatchReplaceDTO) (model.CreateOneToOneSchedulesResult, error) {
	...
	switch existing[0].ClassType {
	case model.TeachingClassTypeOneToOne:
		return repo.replaceOneToOneScheduleBatch(ctx, instID, operatorID, dto, existing, allBatchSchedules, fallbackTeacherID, fallbackClassroomID, removeTeacherFromAssistantIDs(fallbackTeacherID, assistantIDs))
	case model.TeachingClassTypeNormal:
		return repo.replaceGroupClassScheduleBatch(ctx, instID, operatorID, dto, existing, allBatchSchedules, fallbackTeacherID, fallbackClassroomID, assistantIDs)
	default:
		return model.CreateOneToOneSchedulesResult{}, errors.New("当前日程类型暂不支持批次规则编辑")
	}
}

func collectTeachingScheduleIDs(list []model.TeachingScheduleVO) []int64 {
...
func splitTeachingScheduleBatchTargetIDs(existingIDs, allBatchIDs []int64) []int64 {
...
func (repo *Repository) replaceOneToOneScheduleBatch(ctx context.Context, instID, operatorID int64, dto model.TeachingScheduleBatchReplaceDTO, existing, allBatchSchedules []model.TeachingScheduleVO, fallbackTeacherID, fallbackClassroomID int64, assistantIDs []int64) (model.CreateOneToOneSchedulesResult, error) {
	...
	classIDText := strconv.FormatInt(classID, 10)
	if raw := strings.TrimSpace(dto.OneToOneID); raw != "" && raw != classIDText {
	...
	for _, item := range existing {
		...
		if strings.TrimSpace(item.TeachingClassID) != classIDText {
...
func (repo *Repository) replaceGroupClassScheduleBatch(ctx context.Context, instID, operatorID int64, dto model.TeachingScheduleBatchReplaceDTO, existing, allBatchSchedules []model.TeachingScheduleVO, fallbackTeacherID, fallbackClassroomID int64, assistantIDs []int64) (model.CreateOneToOneSchedulesResult, error) {
	classID, err := strconv.ParseInt(strings.TrimSpace(existing[0].TeachingClassID), 10, 64)
	if err != nil || classID <= 0 {
		return model.CreateOneToOneSchedulesResult{}, errors.New("当前批次缺少有效的班级信息")
	}
	classIDText := strconv.FormatInt(classID, 10)
	if raw := strings.TrimSpace(dto.GroupClassID); raw != "" && raw != classIDText {
		return model.CreateOneToOneSchedulesResult{}, errors.New("当前批次与所选班级不一致")
	}
	for _, item := range existing {
		if item.ClassType != model.TeachingClassTypeNormal {
			return model.CreateOneToOneSchedulesResult{}, errors.New("仅支持按班课规则重排当前批次")
		}
		if strings.TrimSpace(item.TeachingClassID) != classIDText {
			return model.CreateOneToOneSchedulesResult{}, errors.New("当前批次包含多个班级，暂不支持合并重排")
		}
	}
	for _, item := range allBatchSchedules {
		if item.ClassType != model.TeachingClassTypeNormal {
	...
	if base.Status != model.TeachingClassStatusActive {
		return model.CreateOneToOneSchedulesResult{}, errors.New("当前班级已结班，暂不可调整批次规则")
	}
	if fallbackTeacherID <= 0 {
		fallbackTeacherID = base.DefaultTeacherID
	}
	...
	classConflicts, err := repo.listScheduleConflictDetailsTx(ctx, tx, instID, "teaching_class_id", base.ClassID, plansToSlots(plans), "", existingIDs)
	if err != nil {
		return model.CreateOneToOneSchedulesResult{}, err
	}
...
func (repo *Repository) BatchUpdateTeachingSchedules(ctx context.Context, instID, operatorID int64, dto model.TeachingScheduleBatchUpdateDTO) error {
	...
	assistantIDs := parseStringIDs(dto.AssistantIDs)
	assistantIDs = removeTeacherFromAssistantIDs(teacherID, assistantIDs)
...
func (repo *Repository) listTeachingScheduleDetailStudentsTx(ctx context.Context, tx *sql.Tx, instID, scheduleID, classID int64, scheduleStartAt time.Time, _ int) ([]model.TeachingScheduleDetailStudentVO, []model.TeachingScheduleDetailStudentVO, error) {
	if scheduleID <= 0 || classID <= 0 {
		return nil, nil, nil
	}
	recordedSummaryMap, err := repo.loadTeachingScheduleRecordedStudentSummaryMap(ctx, tx, instID, []int64{scheduleID})
	if err != nil {
		return nil, nil, err
	}
	recordedStudentSet := recordedSummaryMap[scheduleID].StudentIDSet
	rosterByScheduleID, err := repo.loadEffectiveGroupClassScheduleRosterMap(ctx, tx, instID, []effectiveGroupClassScheduleMeta{{
	...
	buildStudentVO := func(student groupClassScheduleStudent) model.TeachingScheduleDetailStudentVO {
		...
		if student.StudentID > 0 {
			_, hasTeachingRecord = recordedStudentSet[student.StudentID]
		}
		studentCallStatus := 1
		if hasTeachingRecord {
			studentCallStatus = 2
...
func (repo *Repository) FillTeachingScheduleCallStatus(ctx context.Context, instID int64, items []model.TeachingScheduleVO) error {
	...
	for index, item := range items {
		...
		if _, ok := itemIndex[id]; !ok {
			ids = append(ids, id)
			classID, _ := strconv.ParseInt(strings.TrimSpace(item.TeachingClassID), 10, 64)
	...
	statusByID, err := repo.computeTeachingScheduleCallStatusMap(ctx, repo.db, instID, metas)
	if err != nil {
		return err
	}

	for _, id := range ids {
		status := normalizeTeachingScheduleCallStatus(statusByID[id])
		for _, index := range itemIndex[id] {
			items[index].CallStatus = status
			items[index].CallStatusText = teachingScheduleCallStatusText(status)
		}
...
func (roster groupClassScheduleRoster) activeStudentNameMap() map[int64]string {
	result := make(map[int64]string, len(roster.Active))
	for _, student := range roster.Active {
		if student.StudentID <= 0 {
			continue
		}
		result[student.StudentID] = firstNonEmptyString(strings.TrimSpace(student.StudentName), "-")
	}
	return result
}

func (roster groupClassScheduleRoster) overlappingActiveStudentNames(studentIDs map[int64]struct{}) []string {
	if len(studentIDs) == 0 {
		return nil
	}
	result := make([]string, 0, len(roster.Active))
	for _, student := range roster.Active {
		...
		name := firstNonEmptyString(strings.TrimSpace(student.StudentName), "-")
		if name != "" {
			result = append(result, name)
		}
	}
...
func removeTeacherFromAssistantIDs(teacherID int64, assistantIDs []int64) []int64 {
	if len(assistantIDs) == 0 {
		return nil
	}
	result := make([]int64, 0, len(assistantIDs))
	for _, assistantID := range assistantIDs {
		if assistantID <= 0 || (teacherID > 0 && assistantID == teacherID) {
			continue
		}
		result = append(result, assistantID)
	}
...
func collectGroupClassConflictingStudentNames(
	currentRoster groupClassScheduleRoster,
	rows []scheduleConflictDetailRow,
	conflictRosterByScheduleID map[int64]groupClassScheduleRoster,
) []string {
	if len(rows) == 0 {
		return nil
	}
	currentStudentNameMap := currentRoster.activeStudentNameMap()
	if len(currentStudentNameMap) == 0 {
		return nil
	}
	currentStudentIDs := make(map[int64]struct{}, len(currentStudentNameMap))
	for studentID := range currentStudentNameMap {
		currentStudentIDs[studentID] = struct{}{}
	}

	names := make([]string, 0, len(currentStudentNameMap))
	seenNames := make(map[string]struct{}, len(currentStudentNameMap))
	...
	for _, row := range rows {
		if row.ClassType == model.TeachingClassTypeOneToOne {
			appendName(currentStudentNameMap[row.StudentID])
			continue
		}
		if row.ClassType != model.TeachingClassTypeNormal {
			continue
		}
		roster, ok := conflictRosterByScheduleID[row.ID]
		if !ok {
			continue
		}
		for _, name := range roster.overlappingActiveStudentNames(currentStudentIDs) {
			appendName(name)
		}
	}
	return names
...
func buildGroupClassScheduleValidationResultFromPlans(
	base model.GroupClassScheduleCreateContext,
	plans []normalizedSchedulePlan,
	planRosterByKey map[string]groupClassScheduleRoster,
	teacherNames map[int64]string,
	classroomNames map[int64]string,
	teacherConflictsByPlan map[string][]scheduleConflictDetailRow,
	classroomConflictsByPlan map[string][]scheduleConflictDetailRow,
	classConflicts []scheduleConflictDetailRow,
	studentConflictsByPlan map[string][]scheduleConflictDetailRow,
	assistantConflictsByPlan map[string][]scheduleConflictDetailRow,
	conflictRosterByScheduleID map[int64]groupClassScheduleRoster,
) model.TeachingScheduleValidationResult {
	globalTypeSet := make(map[string]struct{})
	globalExistingMap := make(map[int64]model.TeachingScheduleConflictItem)
	...
	for _, plan := range plans {
		...
		itemExistingMap := make(map[int64]model.TeachingScheduleConflictItem)
		conflictTypes := make([]string, 0, 5)

		teacherRows := teacherConflictsByPlan[key]
		if len(teacherRows) > 0 {
			conflictTypes = append(conflictTypes, "老师")
			globalTypeSet["老师"] = struct{}{}
			appendConflictRowsToItemMap(itemExistingMap, teacherRows, "老师")
		...
		classroomRows := classroomConflictsByPlan[key]
		if len(classroomRows) > 0 {
			conflictTypes = append(conflictTypes, "教室")
			globalTypeSet["教室"] = struct{}{}
			appendConflictRowsToItemMap(itemExistingMap, classroomRows, "教室")
		...
		classRows := filterOverlappingScheduleConflictRows(classConflicts, slot.LessonDate, slot.StartAt, slot.EndAt)
		if len(classRows) > 0 {
			conflictTypes = append(conflictTypes, "班级")
			globalTypeSet["班级"] = struct{}{}
			appendConflictRowsToItemMap(itemExistingMap, classRows, "班级")
			appendConflictRowsToItemMap(globalExistingMap, classRows, "班级")
		}

		studentRows := studentConflictsByPlan[key]
		conflictingStudentNames := collectGroupClassConflictingStudentNames(
			planRosterByKey[key],
			studentRows,
			conflictRosterByScheduleID,
		)
		...
		assistantRows := assistantConflictsByPlan[key]
		if len(assistantRows) > 0 {
			conflictTypes = append(conflictTypes, "助教")
			globalTypeSet["助教"] = struct{}{}
			appendConflictRowsToItemMap(itemExistingMap, assistantRows, "助教")
		...
		sort.Strings(conflictTypes)
		currentRoster := planRosterByKey[key]
		currentStudentNames := currentRoster.activeNames()
		if len(currentStudentNames) == 0 {
			currentStudentNames = compactStrings([]string{base.ClassName})
		}
		currentItems = append(currentItems, model.TeachingScheduleConflictItem{
			Name:           base.ClassName,
			ClassTypeText:  scheduleClassTypeText(model.TeachingClassTypeNormal),
			Date:           dateKey,
			Week:           weekDisplay(plan.LessonDate),
			TimeText:       plan.StartAt.Format("15:04") + "~" + plan.EndAt.Format("15:04"),
			TeacherID:      emptyStringIfZero(plan.TeacherID),
		...
		item := model.TeachingScheduleValidationItem{
			...
			Valid:                   len(conflictTypes) == 0,
			ConflictTypes:           conflictTypes,
			ConflictingStudentNames: conflictingStudentNames,
		}
		if !item.Valid {
			item.Message = buildAvailabilityConflictSummaryMessage(conflictTypes)
			item.ExistingSchedules = finalizeConflictItemMap(itemExistingMap)
...
func teachingScheduleStudentActionDisabledReason(hasTeachingRecord bool) string {
	if hasTeachingRecord {
		return "该学员已点名，不可调课或移出本节"
	}
	return ""
}
