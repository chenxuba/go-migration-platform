...
SELECT
  '孤独症儿童发展评估表', 'AUTISMDEV', '标准化测评', '现场测评', '0岁-6岁', '2010-revised-trainer',
...
SELECT s.id, '中国残疾人联合会康复部《孤独症儿童发展评估表》使用手册及评估量表。', 1, NOW(), NOW(), 0, 1
FROM sys_scale s
WHERE s.scale_code = 'AUTISMDEV' AND s.del_flag = 0
  AND NOT EXISTS (
    SELECT 1 FROM sys_scale_reference r
    WHERE r.scale_id = s.id
      AND r.content = '中国残疾人联合会康复部《孤独症儿童发展评估表》使用手册及评估量表。'
      AND r.del_flag = 0
  );
