...
const displayArray = ref(['stuPhoneSearch', 'followTime', 'nextFollowTime', 'visitStatus', 'sex', 'channelCategory', 'followMethod', 'stuStatus'])
...
async function getFollowUpCount() {
  try {
    const res = await getFollowRecordCountApi({ queryAllOrDepartment: 1 })
...
/** 后端 visitStatus 为 bool 或 0/1；null 视为未回访 */
...
<template>
  <div class="tab-content">
    <all-filter
      ref="allFilterRef" :display-array="displayArray" :is-quick-show="true" :student-status="0"
      :follow-up-count="followUpCount" :hide-quick-filters="[2]" :custom-quick-filter-values="{ 3: 2 }" v-on="filterUpdateHandlers"
    />
    <div class="tab-table">
      ...
      <div class="table-content mt-2">
        <a-table
          :data-source="dataSource" row-key="id" :loading="loading" :pagination="pagination"
        ...
        >
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'name'">
              <student-avatar
                :id="record.studentId"
                :name="record.stuName" :gender="record.stuSex == 1 ? '男' : record.stuSex == 0 ? '女' : '未知'"
                :avatar-url="record.avatarUrl" :show-age="false" default-active-key="4"
              />
            </template>
            ...
            <template v-if="column.key === 'followImages'">
              <template v-if="record.followImages && formatFollowImages(record.followImages) !== '-'">
                <a-tooltip title="点击可查看具体内容">
                  <span class="cursor-pointer hover:text-#06f" @click="handleSeeFollowImages(record)">{{ formatFollowImages(record.followImages) }}</span>
                </a-tooltip>
              </template>
            ...
            <template v-if="column.key == 'nextFollowUpTime'">
              <clamped-text
                :class="{ 'text-#f90': isOverdue(record) }"
