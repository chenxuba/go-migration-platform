...
const allowList = ['/platform/login', '/platform/login-template-preview', '/platform/404', '/platform/403']
...
export const useMultiTab = defineStore('multi-tab', () => {
  ...
  const addItem = (route: RouteLocationNormalizedLoaded) => {
    ...
    if (route.path.startsWith('/platform/redirect') || route.path.startsWith('/common'))
      return
    if (route.path === '/')
      return
