...
import { getApprovalMyPagedListApi, getApprovalMyStatisticsCountApi } from '@/api/finance-center/approval-manage'
...
const approveQuickCounts = ref({
  truntoMyApproveCount: 0,
  myHaveApprovedCount: 0,
  initiateApproveCount: 0,
})
...
const queryState = ref({
  ...
  studentId: undefined,
  statuses: undefined,
  truntoMyApprove: undefined,
  myHaveApproved: undefined,
})
...
async function handleOrderDetailDrawerClosed() {
...
async function fetchStatistics() {
  try {
    const res = await getApprovalMyStatisticsCountApi()
    if (res.code === 200 && res.result) {
      approveQuickCounts.value = {
        truntoMyApproveCount: res.result.truntoMyApproveCount || 0,
        myHaveApprovedCount: res.result.myHaveApprovedCount || 0,
        initiateApproveCount: res.result.initiateApproveCount || 0,
      }
      return
    }
  }
  catch (error) {
    console.error('获取我的审批统计失败:', error)
  }
}
...
async function fetchApprovalList(id, type) {
  try {
    loading.value = true
    const res = await getApprovalMyPagedListApi({
    ...
    messageService.error(res.message || '获取我的审批失败')
  }
  catch (error) {
    console.error('获取我的审批失败:', error)
    messageService.error('获取我的审批失败')
  }
  finally {
    loading.value = false
  }
...
const filterUpdateHandlers = computed(() => ({
  ...
  'update:quickFilter': (val, isClearAll, id, type) => handleFilterUpdate({
    truntoMyApprove: val === 1 ? true : undefined,
    myHaveApproved: val === 2 ? true : undefined,
  }, isClearAll, id, type),
}))
  ...
  <div>
    <div class="filter-wrap bg-white pl-3 pr-3 rounded-lb-4 rounded-rb-4">
      <all-filter
        ...
        :is-approve-quick-show="true"
        :is-show-search-stu-phone="true"
        :approve-quick-counts="approveQuickCounts"
