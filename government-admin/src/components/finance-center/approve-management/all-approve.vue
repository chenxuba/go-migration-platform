...
import { getApprovalAllPagedListApi } from '@/api/finance-center/approval-manage'
import messageService from '@/utils/messageService'

const displayArray = ref(['approveNumber', 'createUser', 'orderNumber', 'salesPerson', 'finishTime', 'createTime', 'approvalStatus'])
...
async function fetchApprovalList(id, type) {
  try {
    loading.value = true
    const res = await getApprovalAllPagedListApi({
    ...
    console.error('获取全部审批失败:', error)
    messageService.error('获取全部审批失败')
  }
  finally {
    loading.value = false
  }
...
const filterUpdateHandlers = computed(() => ({
  ...
  'update:createUserFilter': (val, isClearAll, id, type) => handleFilterUpdate({ initiateStaffId: val || undefined }, isClearAll, id, type),
  ...
  'update:createTimeFilter': (val, isClearAll, id, type) => {
    ...
    handleFilterUpdate({ initiateStartTime: currentYearStart, initiateEndTime: today }, isClearAll, id, type)
  },
  ...
  <div>
    <div class="filter-wrap bg-white pl-3 pr-3 rounded-lb-4 rounded-rb-4">
      <all-filter
        ...
        create-user-label="申请人"
        create-user-placeholder="搜索申请人"
