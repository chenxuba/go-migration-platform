...
import { type TeachingScheduleValidationItem, type TeachingScheduleValidationResult, validateGroupClassSchedulesApi } from '@/api/edu-center/teaching-schedule'
...
const validationState = ref<TeachingScheduleValidationResult | null>(props.validation || null)
...
function timeValueForRow(row: { groupKey?: string, startTime?: string, endTime?: string }) {
  return `${String(row.groupKey || '')}|${String(row.startTime || '')}|${String(row.endTime || '')}`
}
...
function groupOptionsForRow(row: { teacherId?: string }) {
  const groups = allowedGroupOptionsByTeacher(String(row.teacherId || ''))
  return groups.length ? groups : (props.periodGroups || [])
}

function timeOptionsForRow(row: { teacherId?: string, groupKey?: string }) {
  const groups = props.periodGroups || []
  const group = groups.find(item => item.key === String(row.groupKey || '')) || groupOptionsForRow(row)[0]
  return group?.timeOptions || []
}

function buildAvailabilityItemKey(teacherId: string, lessonDate: string, startTime: string, endTime: string) {
  return `${normalizedTeacherId(teacherId)}|${lessonDate}|${startTime}|${endTime}`
}

function buildTimeOptionAvailabilityView(item: TeachingScheduleValidationItem): TimeOptionAvailabilityView {
  if (item.valid) {
    return {
      status: 'free',
      statusText: '可排',
      reasonText: '当前节次可安排班课',
      conflictTypes: [],
    }
  }
  ...
  return {
    status: 'busy',
    statusText: conflictTypes.length ? `${conflictTypes.join('、')}冲突` : '有冲突',
    reasonText: item.message || (conflictTypes.length ? `${conflictTypes.join('、')}冲突` : '当前节次不可排'),
    conflictTypes,
  }
}

function timeOptionAvailabilityFor(row: { teacherId?: string, date?: string }, option: TimeOption): TimeOptionAvailabilityView {
  const teacherId = normalizedTeacherId(String(row.teacherId || ''))
  if (!teacherId) {
    return {
      status: 'unknown',
      statusText: '先选老师',
  ...
  const key = buildAvailabilityItemKey(teacherId, String(row.date || ''), option.startTime, option.endTime)
...
function timeOptionViewsForRow(row: { teacherId?: string, groupKey?: string, date?: string, startTime?: string, endTime?: string }): TimeOptionSelectView[] {
  return timeOptionsForRow(row).map((option) => {
    const availability = timeOptionAvailabilityFor(row, option)
    return {
      value: buildTimeValue(String(row.groupKey || ''), option.startTime, option.endTime),
...
const rowViews = computed(() =>
  rowStates.value.map((row, index) => {
    ...
    const hasClassConflict = conflictTypes.includes('班级')
    ...
    const displayHasClassConflict = displayConflictTypes.includes('班级')
    ...
    return {
      ...
      displayHasClassConflict,
      localResolvedConflictTypes,
      confirmedResolvedConflictTypes,
      resolvedConflictTypes,
      canCreate: !hasTeacherConflict && !hasAssistantConflict && !hasClassConflict && !hasClassroomConflict && (!hasStudentConflict || row.allowStudentConflict),
    }
  }),
)
...
const workbenchRowViews = computed(() => rowViews.value.filter(row => shouldRenderRowInWorkbench(row)))
...
const summary = computed(() => {
  ...
  return {
    ...
    class: rows.filter(row => row.hasClassConflict).length,
    ready: allRows.filter(row => row.canCreate).length,
  }
})

const canEnableAllSoftConflicts = computed(() =>
  rowViews.value.some(row => row.displayHasStudentConflict && !row.allowStudentConflict),
)
...
function currentRowAssistantEntries(row: { assistantIds?: Array<string | number>, displayHasAssistantConflict?: boolean }) {
  return normalizedAssistantIds(row.assistantIds).map((id) => {
    const name = assistantNamesByIds([id])[0] || id
    return {
      id,
      name,
      danger: row.displayHasAssistantConflict === true,
    }
  })
}

function selectedAssistantNameSetForRow(row: { assistantIds?: Array<string | number> }) {
  return new Set(assistantNamesByIds(row.assistantIds).map(name => normalizeText(name)).filter(Boolean))
}

function selectedAssistantIdSetForRow(row: { assistantIds?: Array<string | number> }) {
  return new Set(normalizedAssistantIds(row.assistantIds).map(id => normalizedTeacherId(id)).filter(Boolean))
}

function conflictItemAssistantEntries(row: { assistantIds?: Array<string | number> }, item: ConflictDetailView) {
...
function conflictItemTeacherTone(row: { assistantIds?: Array<string | number> }, item: ConflictDetailView) {
...
function assistantOptionsForRow(row: { teacherId?: string }) {
  return (props.assistantOptions || []).filter(option => option.value !== String(row.teacherId || ''))
}

function buildValidationPayload(options?: { schedules?: Array<{
  lessonDate: string
  startTime: string
  endTime: string
  teacherId?: string
  ...
  const schedules = options?.schedules || rowStates.value.map(row => ({
    lessonDate: row.date,
  ...
  const unionAssistantIds = Array.from(new Set(schedules.flatMap(row => normalizedAssistantIds(row.assistantIds))))
  return {
    groupClassId: props.groupClassId,
    teacherId: props.defaultTeacherId || schedules[0]?.teacherId || '',
...
async function revalidateRows() {
  if (!props.groupClassId || !rowStates.value.length)
    return
  validating.value = true
  try {
    const res = await validateGroupClassSchedulesApi(buildValidationPayload())
    ...
    console.error('revalidate group class rows failed', error)
...
async function fetchTimeOptionAvailability() {
  ...
  const groupClassId = String(props.groupClassId || '').trim()
  ...
  rowStates.value.forEach((row) => {
    ...
    timeOptionsForRow(row).forEach((option) => {
      const key = buildAvailabilityItemKey(teacherId, row.date, option.startTime, option.endTime)
  ...
  if (!props.open || !groupClassId || !schedulesMap.size || schedulesMap.size > 2000) {
  ...
  try {
    ...
    ;(res.result.items || []).forEach((item) => {
      nextMap[buildAvailabilityItemKey(item.teacherId || '', item.lessonDate, item.startTime, item.endTime)] = buildTimeOptionAvailabilityView(item)
    })
    ...
    if (seq !== timeOptionAvailabilitySeq)
      return
    console.error('fetch group class time option availability failed', error)
    timeOptionAvailabilityMap.value = {}
  }
  finally {
    if (seq === timeOptionAvailabilitySeq)
...
function initializeRows() {
  rowStates.value = props.plans.map((plan, index) => {
    ...
    return {
      ...
      assistantIds: normalizedAssistantIds(plan.assistantIds || []),
...
watch(
  () => [
    props.open ? '1' : '0',
    props.groupClassId,
    rowStates.value.map(row => `${row.key}|${row.date}|${row.teacherId}|${row.startTime}|${row.endTime}|${row.classroomId}|${row.assistantIds.join(',')}`).join(','),
  ].join('|'),
  () => {
    if (props.open)
      scheduleTimeOptionAvailabilityCheck()
...
function updateRow<K extends keyof WorkbenchRow>(key: string, field: K, value: WorkbenchRow[K]) {
  rowStates.value = rowStates.value.map((row) => {
    ...
    if (field === 'teacherId') {
      ...
      next.assistantIds = normalizedAssistantIds(next.assistantIds).filter(id => id !== next.teacherId)
    }
    if (field === 'groupKey') {
      const allowedTimeOptions = timeOptionsForRow(next)
  ...
  if (field === 'teacherId' || field === 'groupKey')
    scheduleTimeOptionAvailabilityCheck()
  if (field === 'teacherId' || field === 'classroomId' || field === 'startTime' || field === 'endTime' || field === 'groupKey' || field === 'allowStudentConflict')
    scheduleRevalidate()
}
  ...
  >
    ...
    <div class="schedule-conflict">
      <div class="schedule-conflict__banner">
        ...
        <span>{{ validationState?.message || '当前创建班课日程存在冲突' }}</span>
      </div>

      <div class="schedule-conflict__toolbar">
        <div class="schedule-conflict__toolbar-summary">
          共 {{ summary.total }} 节待处理日程，其中班级冲突 {{ summary.class }} 节，老师冲突 {{ summary.teacher }} 节，助教冲突 {{ summary.assistant }} 节，学员冲突 {{ summary.student }} 节，教室冲突 {{ summary.classroom }} 节，当前可直接创建 {{ summary.ready }} 节。
        </div>
          ...
          <template v-if="column.key === 'current'">
            <div class="schedule-conflict__cell-card">
              ...
              <div class="schedule-conflict__cell-main">
                ...
                <span>{{ record.current?.classTypeText || '班课日程' }}</span>
              ...
              <div class="schedule-conflict__cell-meta">
                ...
                <span>班级学员：<strong :class="{ 'schedule-conflict__cell--danger': record.displayHasStudentConflict }">{{ (record.current?.studentNames || []).join('、') || '-' }}</strong></span>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="column.key === 'actions'">
            <div class="schedule-conflict__action-panel">
              ...
              <div class="schedule-conflict__action-group">
                <div class="schedule-conflict__action-label-row">
                  ...
                  <small class="schedule-conflict__action-label-hint">
                    {{ timeOptionAvailabilityLoading ? '正在检测当前节次是否可排' : '先选老师，再看节次可排情况' }}
                  </small>
                </div>
                <a-select
              ...
              <div class="schedule-conflict__action-group">
                ...
                >
                  <a-select-option
                    v-for="item in assistantOptionsForRow(record)"
                    :key="item.value"
                    :value="item.value"
                    :label="item.label"
                  >
                    <div class="schedule-conflict__assistant-option">
                      <div class="schedule-conflict__assistant-option-main">
                        <span class="schedule-conflict__assistant-option-label">{{ item.label }}</span>
                        <span v-if="item.mobile" class="schedule-conflict__assistant-option-mobile">{{ item.mobile }}</span>
                      </div>
                    </div>
                  </a-select-option>
                </a-select>
              ...
              >
                ...
                <div class="schedule-conflict__action-option-main">
                  <span>忽略学员冲突</span>
                  <small>允许班级学员并行上课，创建后保留冲突标记</small>
                </div>
              </label>

              <div v-if="record.displayHasClassConflict" class="schedule-conflict__action-tip schedule-conflict__action-tip--danger">
                <span class="schedule-conflict__action-badge schedule-conflict__action-badge--danger">班级冲突</span>
                <span>请调整节次时间后再创建</span>
              </div>
              ...
              <div v-if="record.displayHasAssistantConflict" class="schedule-conflict__action-tip schedule-conflict__action-tip--danger">
                ...
                <span>请调整助教或节次后再创建</span>
              </div>
              <div v-if="record.displayHasClassroomConflict" class="schedule-conflict__action-tip schedule-conflict__action-tip--danger">
                <span class="schedule-conflict__action-badge schedule-conflict__action-badge--danger">教室冲突</span>
                <span>请更换教室或调整节次后再创建</span>
              </div>
              <div
                v-else-if="record.resolvedConflictTypes.length"
              ...
              >
                ...
                <span>部分原始冲突已解除，最终结果以最新校验为准</span>
              </div>
      ...
      <div class="schedule-conflict__footer">
        <div class="schedule-conflict__footer-hint">
          班课仅支持忽略学员冲突；班级、老师、助教、教室冲突请先调整后再创建。
        </div>
...
.schedule-conflict__assistant-option {
  display: flex;
  align-items: center;
  justify-content: space-between;
  width: 100%;
...
.schedule-conflict__assistant-option-main {
  display: flex;
  align-items: center;
  gap: 10px;
  min-width: 0;
...
.schedule-conflict__assistant-option-label {
  color: #1f2329;
  font-size: 14px;
  font-weight: 600;
}

.schedule-conflict__assistant-option-mobile {
  color: #98a2b3;
  font-size: 12px;
}
