...
import { ref } from "vue";
import {
  getGroupClassUnscheduledRollCallContextApi,
  type GroupClassUnscheduledRollCallContextParams,
  type GroupClassUnscheduledRollCallContextResult,
...
const rollCallContext = ref<GroupClassUnscheduledRollCallContextResult | null>(
  null
);

async function handleStartRollCall(
  payload: GroupClassUnscheduledRollCallContextParams
) {
  try {
    ...
    const studentCount = Array.isArray(res.result.record?.students)
      ? res.result.record.students.length
      : 0;
    if (studentCount <= 0) {
      const className =
        String(res.result.detail?.className || "").trim() || "当前班级";
