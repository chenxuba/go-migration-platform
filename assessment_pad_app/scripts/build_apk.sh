...
if [[ "$BUILD_TARGET" == "prod" || "$BUILD_TARGET" == "production" || "$BUILD_TARGET" == "online" ]]; then
  ...
  TRAINING_GAME_BASE_URL="${TRAINING_GAME_BASE_URL:-$PROD_SCHEME://$PROD_DOMAIN/training-games/}"
...
echo "Using TRAINING_GAME_BASE_URL=${TRAINING_GAME_BASE_URL:-}"
...
DART_DEFINES=(
  ...
  "--dart-define=TRAINING_GAME_BASE_URL=${TRAINING_GAME_BASE_URL:-}"
