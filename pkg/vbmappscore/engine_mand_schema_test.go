...
func TestScoreAutoCalculatesMAND08MFromSharedObservationEvidence(t *testing.T) {
	...
	if item.Score == nil || *item.Score != 1 {
		t.Fatalf("expected MAND_08M score 1 from shared observation evidence, got %+v", item)
	}
}

func TestScoreAutoCalculatesMAND09MFromSharedObservationEvidenceWithSemanticDedup(t *testing.T) {
	domains, milestones, rules, barriers, transitions := loadGeneratedDrafts(t)
	engine, err := NewEngine(domains, milestones, rules, barriers, transitions)
	if err != nil {
		t.Fatalf("NewEngine: %v", err)
	...
	if item.Score == nil || *item.Score != 0.5 {
		t.Fatalf("expected MAND_09M score 0.5 from shared observation evidence with semantic dedup, got %+v", item)
	}
}
