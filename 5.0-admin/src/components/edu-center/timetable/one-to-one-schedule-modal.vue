...
watch(
  ...
  async ([open, preset]) => {
    if (!open || !preset || !oneToOneRecords.value.length)
      return
...
const schoolSlotTooltipText = computed(() =>
  isSingleScheduleEditMode.value
    ? '单个1对1日程编辑时仅允许选择一个课表节次，保存后会更新当前这节日程。'
    : '同一上课日内按所选「第几节课」各生成一节日程；重复排课时每个上课日都会生成这些节，总节数仍受剩余课时/额度约束。',
)
...
const modalTitleText = computed(() => isBatchPlanEditMode.value ? '编辑1对1日程' : '创建1对1日程')

const modalSubtitleText = computed(() =>
  isSingleScheduleEditMode.value
    ? '回显当前1对1日程信息，可调整本节的日期与时间资源。'
    : (
...
const selectedRecordPlaceholderText = computed(() => {
  if (oneToOneLoading.value)
    return '正在加载1对1数据...'
  return isSingleScheduleEditMode.value ? '当前日程对应的1对1' : (isBatchPlanEditMode.value ? '当前批次对应的1对1' : '请选择1对1')
})

const summaryCardTitleText = computed(() => isSingleScheduleEditMode.value ? '当前日程' : (isBatchPlanEditMode.value ? '当前批次' : '当前档案'))
const summaryCardDescText = computed(() =>
  isSingleScheduleEditMode.value
    ? '来自当前日程的基础信息与编辑摘要。'
    : (isBatchPlanEditMode.value ? '来自当前批次的基础信息与规则摘要。' : '来自当前1对1记录的基础信息与创建摘要。'),
)
...
const formCardDescText = computed(() =>
  isSingleScheduleEditMode.value
    ? '当前为单节1对1编辑，可调整开始日期与时间资源，其他日期规则已锁定。'
...
const reviewSubtitleText = computed(() =>
  isSingleScheduleEditMode.value
    ? '先确认本次将保存的日程，再执行保存。'
    : (isBatchPlanEditMode.value ? '先确认本次将替换出的日程，再执行整体保存。' : '先确认本次将创建的日程，再执行批量创建。'),
)
...
const datePlanFooterHintText = computed(() =>
  isSingleScheduleEditMode.value ? '当前为单节1对1编辑，开始日期可以调整，计划次数固定为 1。' : `可自由填写节数；结束日期由开始日期、重复规则与本次数推算。参考剩余可排 ${remainSessionCap.value} 节。`,
)
...
async function confirmBatchCreate(options: {
  ...
  try {
    ...
    if (res.code !== 200)
      throw new Error(res.message || (isSingleScheduleEditMode.value ? '保存1对1日程失败' : (isBatchPlanEditMode.value ? '保存批次规则失败' : '创建1对1日程失败')))
    const count = res.result?.count || (options.plans?.length || previewPlans.value.length)
    if (isBatchPlanEditMode.value) {
      messageService.success(isSingleScheduleEditMode.value ? '已更新1对1日程' : `已按新规则替换 ${count} 节日程`)
    }
    ...
    previewHasConflict.value = true
    previewValidationMessage.value = error?.response?.data?.message || error?.message || (isSingleScheduleEditMode.value ? '保存1对1日程失败' : (isBatchPlanEditMode.value ? '保存批次规则失败' : '创建1对1日程失败'))
    if (previewValidationResult.value)
      conflictModalOpen.value = true
