...
export type BatchPlanScheduleType = 'oneToOne'
export type BatchPlanSchedulingMode = 'repeat' | 'free'
export type BatchPlanRepeatRule = 'none' | 'weekly' | 'biweekly' | 'daily' | 'alternateDay'
export type BatchPlanHolidayPolicy = 'include' | 'filter'

export interface BatchPlanTimeBlockPreset {
  startTime: string
  endTime: string
}
...
export interface BatchPlanModalPreset {
  ...
  repeatRule: BatchPlanRepeatRule
  holidayPolicy: BatchPlanHolidayPolicy
  ...
  assistantIds: string[]
  classroomId?: string
  timeBlocks: BatchPlanTimeBlockPreset[]
  warnings: string[]
  editable: boolean
...
function uniqueTimeBlocks(list: TeachingScheduleItem[]): BatchPlanTimeBlockPreset[] {
  const seen = new Set<string>()
  const result: BatchPlanTimeBlockPreset[] = []
...
function buildFreePairs(dates: string[], timeBlocks: BatchPlanTimeBlockPreset[]) {
...
function normalizeStoredBatchMeta(meta?: TeachingScheduleBatchMeta) {
  ...
  const repeatRule = (meta.repeatRule || 'none') as BatchPlanRepeatRule
  return {
    schedulingMode,
    repeatRule,
...
function inferModeAndRule(list: TeachingScheduleItem[], timeBlocks: BatchPlanTimeBlockPreset[]) {
  ...
  // 规则元数据当前未单独落库，回显时只能从已生成日期反推。
  // 对于「周一到周四」这类连续日期，weekly / daily 都可能命中；
  // 这里优先回显 daily / alternateDay，尽量贴近用户当初选择的“连续生成”语义。
  const repeatCandidates: BatchPlanRepeatRule[] = ['daily', 'alternateDay', 'weekly', 'biweekly', 'none']
...
export function inferBatchPlanPreset(detail: TeachingScheduleBatchDetail, anchorScheduleId?: string): BatchPlanModalPreset {
  ...
  if (storedMeta) {
    return {
      ...
      oneToOneId: detail.teachingClassId,
      scheduleType: 'oneToOne',
      schedulingMode: storedMeta.schedulingMode === 'free' ? 'free' : 'repeat',
      repeatRule: (storedMeta.repeatRule as BatchPlanRepeatRule) || 'none',
