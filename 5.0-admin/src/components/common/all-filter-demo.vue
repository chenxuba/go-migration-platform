...
const emit = defineEmits(['update:channelTypeFilter', 'update:channelStatusFilter', 'update:channelCategoryFilter',
  ...
  'update:hobbiesFilter', 'update:gradeFilter', 'update:stuPhoneSearchFilter', 'update:tuiJianUserFilter',
  ...
  'update:orderTypeFilter', 'update:departmentFilter', 'staff-search', 'update:createUserFilter',
...
async function getDepartmentList() {
  try {
    ...
    if (res.code === 200 && res.result) {
      ...
      // console.log('处理后的部门数据:', dptListOptions.value)
    }
  } catch (error) {
...
function handleTrialPurchaseStatusChange() {
  nextTick(() => {
    console.log('体验课购买状态:', toRaw(trialPurchaseStatusVals.value))
  })
}
...
function handleCourseAttributeChange(e) {
  nextTick(() => {
    console.log('课程属性:', e.propertyId, e.id)
  })
}
// 渠道树下拉框可见状态 请求数据
          ...
          <div class="standard-filters">
            ...
            <template v-for="(filterType, index) in displayArray" :key="filterType + index">
              ...
              <checkbox-filter v-if="filterType === 'createUser'" :ref="(el) => handleRef(el, 'createUser')"
                v-model:checked-values="createUserVals" category="teacher" placeholder="请输入创建人"
                :options="createUserOptions" label="创建人" type="radio" :finished="finished"
              ...
              <checkbox-filter v-if="filterType === 'salesPerson'" :ref="(el) => handleRef(el, 'salesPerson')"
                v-model:checked-values="selectSalesPersonVals" category="teacher" placeholder="请输入销售员"
                :options="salesPersonOptions" label="销售员" type="radio" :finished="salesPersonFinished"
